...
const AddPermission = () => {
  ...
  const handleRemoveReplacementSlot = (id: number) => {
    if (replacementSlots.length > 1) {
      setReplacementSlots(replacementSlots.filter(slot => slot.id === id));
    }
  };
  
  const handleReplacementChange = (id: number, field: keyof ReplacementSlot, value: string) => {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Ajouter une permission</h1>
      </div>
            ...
            <div className="flex">
              ...
              <div>
                ...
                <ul className="list-disc pl-5 mt-1 space-y-1 text-sm">
                  ...
                  <li>Les permissions doivent être prises par tranches de 30 minutes.</li>
          ...
          <div className="border-t border-gray-200 pt-4">
            ...
            <div className="space-y-4 mb-4">
              {replacementSlots.map((slot, index) => (
                <div key={slot.id} className="border rounded-md p-4 bg-gray-50">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {replacementSlots.length > 1 && (
                      <button
                        type="button"
                        onClick={() => typeof slot.id === 'number' && handleRemoveReplacementSlot(slot.id)}
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div>
                      ...
                      <input
                        type="date"
                        value={slot.date}
                        onChange={(e) => typeof slot.id === 'number' && handleReplacementChange(slot.id, 'date', e.target.value)}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => typeof slot.id === 'number' && handleReplacementChange(slot.id, 'startTime', e.target.value)}
                        className="form-input text-sm"
                        required
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        type="time"
                        value={slot.endTime}
                        onChange={(e) => typeof slot.id === 'number' && handleReplacementChange(slot.id, 'endTime', e.target.value)}
                        className="form-input text-sm"
                        required
                      />
                    </div>
